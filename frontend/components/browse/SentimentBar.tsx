...
export default function SentimentBar({ positive, neutral, negative, showLabels, className = '' }: SentimentBarProps) {
    ...
    return (
        <div className={`w-full ${className}`}>
            ...
            {showLabels && (
                <div className="flex justify-between text-[10px] sm:text-xs mt-1 text-gray-500 font-medium">
                    <span className="text-emerald-600 dark:text-emerald-400">{Math.round(posPct)}% Positive</span>
                    {negPct > 5 && <span className="text-rose-500 dark:text-rose-400">{Math.round(negPct)}% Critiques</span>}
                </div>
            )}
        </div>
    );
