...
def _load_tool_registry() -> Dict[str, Any]:
    ...
    for tool_name in contracts.keys():
        try:
            ...
            logger.warning(f"⚠️  Failed to load tool {tool_name}: {e}")
            continue

    logger.info(f"📦 Loaded {len(registry)} tools dynamically")
    return registry
...
class PlanExecutor:
    ...
    async def execute(self, plan: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not plan or "steps" not in plan or not plan["steps"]:
            logger.error(f"❌ Plan Executor received invalid or empty plan: {plan}")
            return {
        ...
        logger.info(f"🚀 Plan Executor starting with {len(plan['steps'])} steps")
        ...
        logger.info(f"🔍 DEBUG: Plan Executor received slots: {slots}")
    ...
    async def _execute_step(self, step: Dict[str, Any]) -> None:
        ...
        if parallel and len(tool_names) > 1:
            ...
            logger.info(f"  ⚡ Running {len(tool_names)} tools in parallel")
        ...
        for callback in callbacks:
            try:
                if asyncio.iscoroutinefunction(callback):
                    ...
                    logger.info(f"📮 Called async callback for tool citation: {tool_name}")
                else:
                    callback(citation)
                    logger.info(f"📮 Called sync callback for tool citation: {tool_name}")
            except Exception as e:
                logger.error(f"❌ Error calling citation callback: {e}", exc_info=True)
    ...
    def _resolve_args(self, tool_call: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Examples:
            {"args": {"query": "nike shoes"}} → {"query": "nike shoes"}
            {"args_from": "search.product_search.products"} → {products from context}
            {"args_from": "all"} → {flattened context}
        """
    ...
    def _extract_results(self) -> Dict[str, Any]:
        ...
        if self.tool_citations:
            ...
            logger.info(f"📋 Collected {len(self.tool_citations)} tool citations")

        logger.info(f"📦 Extracted results: assistant_text={bool(results.get('assistant_text'))}, ui_blocks={len(results.get('ui_blocks', []))}, citations={len(results.get('citations', []))}")
