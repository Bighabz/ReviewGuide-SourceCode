"""
SerpAPI Client for Review Search

Runs parallel searches across multiple SerpAPI engines to find real product reviews
...
class SerpAPIClient:
    """Client for searching product reviews via SerpAPI."""
    ...
    async def search_reviews(
        ...
        Runs 3 parallel SerpAPI searches:
        1. Google Search: editorial review sites
        2. Google Search: Reddit discussions
        3. Google Shopping: ratings and review counts

        Returns a ReviewBundle with aggregated review data.
        Falls back to empty bundle on any failure.
        """
        # Check cache first
        cached = await self._get_cached(product_name, category)
        if cached:
            logger.info(f"[serpapi] Cache hit for '{product_name}'")
            return cached

        logger.info(f"[serpapi] Searching reviews for '{product_name}' (category: {category or 'general'})")
        ...
        try:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    logger.warning(f"[serpapi] {search_names[i]} search failed: {result}")
                    continue
                if i < 2:
                    # Google search results (editorial + reddit)
            ...
            bundle = ReviewBundle(
                ...
                consensus="",  # Will be generated by product_compose via LLM
            )
            ...
            logger.info(
                f"[serpapi] Found {len(unique_sources)} sources for '{product_name}' "
                f"(avg_rating={avg_rating}, total_reviews={total_reviews})"
            )
            return bundle

        except Exception as e:
            logger.error(f"[serpapi] Search failed for '{product_name}': {e}", exc_info=True)
    ...
    async def _search_editorial(self, product_name: str, category: str) -> List[ReviewSource]:
        ...
        results = await self._serpapi_google_search(query)
    ...
    async def _search_shopping(self, product_name: str) -> Dict[str, Any]:
        ...
        try:
            results = await self._serpapi_shopping_search(product_name)
            ...
            if best.get("reviews"):
                try:
                    review_count = int(str(best["reviews"]).replace(",", ""))
                except (ValueError, TypeError):
                    pass
    ...
    async def _serpapi_google_search(self, query: str) -> Dict[str, Any]:
        """Execute a Google search via SerpAPI."""
        return await self._serpapi_request({
            "engine": "google",
            "q": query,
            "num": 10,
    ...
    async def _serpapi_shopping_search(self, query: str) -> Dict[str, Any]:
    ...
    async def _serpapi_request(self, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _parse_organic_results(self, results: Dict[str, Any]) -> List[ReviewSource]:
        """Parse organic Google search results into ReviewSource objects."""
        sources = []
        ...
        for item in organic:
            ...
            if rich_snippet:
                ...
                if detected.get("reviews"):
                    try:
                        review_count = int(str(detected["reviews"]).replace(",", ""))
                    except (ValueError, TypeError):
                        pass
    ...
    async def _get_cached(self, product_name: str, category: str) -> Optional[ReviewBundle]:
        ...
        try:
            ...
            logger.warning(f"[serpapi] Cache read failed: {e}")
        return None
    ...
    async def _set_cached(self, product_name: str, category: str, bundle: ReviewBundle) -> None:
        ...
        try:
            ...
            logger.warning(f"[serpapi] Cache write failed: {e}")
